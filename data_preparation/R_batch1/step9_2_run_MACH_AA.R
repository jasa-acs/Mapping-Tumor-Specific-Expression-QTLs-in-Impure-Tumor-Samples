...
sh   = "_run_MACH_AA_new.sh"
cat("", file=sh)

for(c1 in 1:22){
  
  ffs  = list.files(path="../data_AA", pattern=sprintf("chr%d.[[:digit:]]+.hap", c1))
  ks   = length(ffs)
  ...
  for(k1 in 1:ks){
    ...
    cmd  = sprintf("%s -d ../data_AA/genotype_marker_chr%d.txt", cmd, c1)
    cmd  = sprintf("%s -p ../data_AA/genotype_ped_chr%d_forward.txt", cmd, c1)
    cmd  = sprintf("%s -s ../data_AA/chr%d.%d.snps", cmd, c1, k1)
    cmd  = sprintf("%s -h ../data_AA/chr%d.%d.hap",  cmd, c1, k1)
    cmd  = sprintf("%s --r 100 --phase --autoflip --greedy --compact", cmd)
    cmd  = sprintf("%s --states 200 -o ../MACH_output_AA/chr%d_%d.out", cmd, c1, k1)
    cmd  = sprintf("%s --forceImputation > AA_chr%d_%d.log\n",   cmd, c1, k1)
    
    sh1  = sprintf("AA_chr%d_%d.sh", c1, k1)
    cat(cmd, file=sh1)
